...
use syn::{parse_macro_input, DeriveInput, GenericArgument, Type};
...
pub fn multi_index_map(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    let lookup_table_fields = fields_to_index().map(|f| {
        ...
        let (ordering, storage_mode) = get_index_kind(f).unwrap_or_else(|| {
            abort_call_site!("Attributes must be in the style #[multi_index(hashed_unique)]")
        });
        let ty = &get_key_type(&f.ty, &storage_mode);

        match storage_mode {
            StorageMode::Unique => match ordering {
                Ordering::Hashed => quote! {
                    #[cfg(test)]
                    #vis #index_name: rustc_hash::FxHashMap<#ty, usize>,
                    #[cfg(not(test))]
                    #index_name: rustc_hash::FxHashMap<#ty, usize>,
                },
                Ordering::Ordered => quote! {
                    #[cfg(test)]
                    #vis #index_name: std::collections::BTreeMap<#ty, usize>,
                    #[cfg(not(test))]
                    #index_name: std::collections::BTreeMap<#ty, usize>,
                },
            },
            StorageMode::NonUnique | StorageMode::IterValue => match ordering {
                Ordering::Hashed => quote! {
                    #[cfg(test)]
                    #vis #index_name: rustc_hash::FxHashMap<#ty, std::collections::BTreeSet<usize>>,
                    #[cfg(not(test))]
                    #index_name: rustc_hash::FxHashMap<#ty, std::collections::BTreeSet<usize>>,
                },
                Ordering::Ordered => quote! {
                    #[cfg(test)]
                    #vis #index_name: std::collections::BTreeMap<#ty, std::collections::BTreeSet<usize>>,
                    #[cfg(not(test))]
    ...
    let lookup_table_fields_init: Vec<proc_macro2::TokenStream> = fields_to_index()
        .map(|f| {
            let index_name = format_ident!("_{}_index", f.ident.as_ref().unwrap());
            let (ordering, _storage_mode) = get_index_kind(f).unwrap_or_else(|| {
                abort_call_site!("Attributes must be in the style #[multi_index(hashed_unique)]")
            });
            match ordering {
                Ordering::Hashed => quote! {
    ...
    let inserts: Vec<proc_macro2::TokenStream> = fields_to_index()
        .map(|f| {
            ...
            let (_ordering, storage_mode) = get_index_kind(f).unwrap_or_else(|| {
                abort_call_site!("Attributes must be in the style #[multi_index(hashed_unique)]")
            });

            match storage_mode {
                StorageMode::Unique => quote! {
                ...
                StorageMode::NonUnique => quote! {
                    self.#index_name.entry(elem.#field_name.clone()).or_insert(std::collections::BTreeSet::new()).insert(idx);
                },
                StorageMode::IterValue => quote! {
                    for item in elem.#field_name.iter(){
                        self.#index_name.entry(item.clone()).or_insert(std::collections::BTreeSet::new()).insert(idx);
                    }
                },
            }
        })
    ...
    let removes: Vec<proc_macro2::TokenStream> = fields_to_index().map(|f| {
        ...
        match storage_mode {
            ...
            StorageMode::IterValue => quote! {
                let key_to_remove = &elem_orig.#field_name;
                for item in key_to_remove.iter() {
                    if let Some(elems) = self.#index_name.get_mut(item) {
                        if elems.len() > 1 {
                            if !elems.remove(&idx){
                                panic!(#error_msg);
                            }
                        } else {
                                self.#index_name.remove(item);
                        }
                    }
                }
            }
    ...
    let modifies: Vec<proc_macro2::TokenStream> = fields_to_index().map(|f| {
        ...
        match storage_mode {
            ...
            StorageMode::IterValue => {
                let get_diff = quote!{
                    let (loss, new) = (
                        elem_orig.#field_name.difference(&elem.#field_name),
                        elem.#field_name.difference(&elem_orig.#field_name),
                    );
                };
                quote! {
                    if elem.#field_name != elem_orig.#field_name {
                        #get_diff
                        for item in loss {
                            let idxs = self.#index_name.get_mut(item).expect(#error_msg);
                            if idxs.len() > 1 {
                                if !(idxs.remove(&idx)) {
                                    panic!(#error_msg);
                                }
    ...
    let accessors = fields_to_index().map(|f| {
        ...
        let mut_getter = match storage_mode {
            StorageMode::Unique => quote! {
                /// SAFETY:
        ...
        let remover = match storage_mode {
            StorageMode::Unique => quote! {
        ...
        let modifier = match storage_mode {
            StorageMode::Unique => quote! {
    ...
    let iterators = fields_to_index().map(|f| {
        ...
        let iter_type = match storage_mode {
            StorageMode::Unique => match ordering {
        ...
        let iter_action = match storage_mode {
            StorageMode::Unique => quote! { Some(&self._store_ref[*self._iter.next()?.1]) },
            StorageMode::NonUnique|StorageMode::IterValue => quote! {
        ...
        let rev_iter_action = match storage_mode {
            StorageMode::Unique => quote! {
...
fn get_index_kind(f: &syn::Field) -> Option<(Ordering, StorageMode)> {
    ...
    if nested_path.is_ident("hashed_unique") {
        Some((Ordering::Hashed, StorageMode::Unique))
    } else if nested_path.is_ident("ordered_unique") {
        Some((Ordering::Ordered, StorageMode::Unique))
    } else if nested_path.is_ident("hashed_non_unique") {
        Some((Ordering::Hashed, StorageMode::NonUnique))
    } else if nested_path.is_ident("ordered_non_unique") {
        Some((Ordering::Ordered, StorageMode::NonUnique))
    } else if nested_path.is_ident("hashed_iter") {
        Some((Ordering::Hashed, StorageMode::IterValue))
    } else if nested_path.is_ident("ordered_iter") {
        Some((Ordering::Ordered, StorageMode::IterValue))
    } else {
        None
    }
}

const SUPPORT_SET: [&str; 3] = ["HashSet", "BTreeSet", "FxHashSet"];
// Get generics of TypePath
fn get_generics_type(itype: &Type) -> Type {
    if let syn::Type::Path(tp) = itype {
        if let Some(segment) = tp.path.segments.last() {
            if SUPPORT_SET.contains(&segment.ident.to_string().as_str()) {
                if let syn::PathArguments::AngleBracketed(ab) = &segment.arguments {
                    if ab.args.len() == 1 {
                        if let GenericArgument::Type(ty) = ab.args.first().as_ref().unwrap() {
                            return ty.clone();
                        }
                    }
                }
    ...
    abort_call_site!("Only support for TypePath like {:?}.", SUPPORT_SET)
}

fn get_key_type(itype: &Type, storage_mode: &StorageMode) -> Type {
    match storage_mode {
        StorageMode::Unique | StorageMode::NonUnique => itype.clone(),
        StorageMode::IterValue => get_generics_type(itype),
    }
}
